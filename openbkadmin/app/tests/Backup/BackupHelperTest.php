...
namespace Tests\OpenBKAdmin\Backup;

use GuzzleHttp\Exception\ConnectException;
use GuzzleHttp\Psr7\Request;
use PHPUnit\Framework\TestCase;
use OpenBKAdmin\Backup\BackupHelper;
use OpenBKAdmin\Backup\BackupResult;
use OpenBKAdmin\Backup\BackupResults;
...
class BackupHelperTest extends TestCase
{
    ...
    {
        $this->tempDir = sys_get_temp_dir().'/OpenBKAdmin-backup-helper-'.bin2hex(random_bytes(6));
    ...
    public function testBackupCreatesZipAndSuccessfulResults(): void
    {
        ...
        $OpenBeken = $this->createMock(OpenBeken::class);
        $OpenBeken->expects(self::once())
            ->method('backup')
            ->with($device, $this->backupPath)
            ->willReturn($backupDump)
        ;

        $helper = new BackupHelper($repository, $OpenBeken, $this->backupPath);
    ...
    public function testBackupSkipsUnknownDeviceIdsAndStillCreatesEmptyZip(): void
    {
        ...
        $OpenBeken = $this->createMock(OpenBeken::class);
        $OpenBeken->expects(self::never())->method('backup');
    ...
    public function testRestoreRejectsMultipleDevices(): void
    {
        $helper = new BackupHelper(
            ...
            $this->createMock(OpenBeken::class),
            $this->backupPath,
            $this->createConfig(),
            '/_BASEURL_/',
            null,
    ...
    public function testRestoreRejectsNonUploadedTempFilesByDefault(): void
    {
        ...
        $OpenBeken = $this->createMock(OpenBeken::class);
        $OpenBeken->expects(self::never())->method('restore');
