const test = require("node:test");
const assert = require("node:assert/strict");

const {
  detectDevicePlatform,
  determineUpgradePlan,
  resolveUpdateTarget,
  shouldTreatStatusAsSuccessful,
  getFailureDetails,
} = require("../../resources/js/device_update_logic.js");

test("manual uploads stay pending while the device reports its pre-update version", () => {
  assert.equal(
    shouldTreatStatusAsSuccessful({
      beforeVersion: "14.4.0(TasmoCompiler-esp8266generic)",
      currentVersion: "14.4.0(TasmoCompiler-esp8266generic)",
      targetVersion: "",
    }),
    false,
  );
});

test("manual uploads succeed once the reported version changes", () => {
  assert.equal(
    shouldTreatStatusAsSuccessful({
      beforeVersion: "14.4.0(TasmoCompiler-esp8266generic)",
      currentVersion: "14.4.1(TasmoCompiler-esp8266generic)",
      targetVersion: "",
    }),
    true,
  );
});

test("targeted uploads still require the exact target version", () => {
  assert.equal(
    shouldTreatStatusAsSuccessful({
      beforeVersion: "14.4.0(OpenBeken)",
      currentVersion: "14.4.1(OpenBeken)",
      targetVersion: "14.4.2",
    }),
    false,
  );
});

test("manual upload failures report the unchanged version context", () => {
  assert.deepEqual(
    getFailureDetails({
      beforeVersion: "14.4.0(OpenBeken)",
      currentVersion: "14.4.0(OpenBeken)",
      targetVersion: "",
    }),
    {
      key: "BLOCK_UPDATE_ERROR_VERSION_NOT_CHANGED",
      values: ["14.4.0(OpenBeken)"],
    },
  );
});

test("platform detection keeps ESP8266 devices on ESP8266 firmware targets", () => {
  assert.equal(
    detectDevicePlatform({
      StatusFWR: {
        Hardware: "ESP8285",
        Version: "14.4.0(OpenBeken)",
      },
    }),
    "esp8266",
  );
});

test("platform detection identifies ESP32 devices from hardware info", () => {
  assert.equal(
    detectDevicePlatform({
      StatusFWR: {
        Hardware: "ESP32-S3",
        Version: "14.4.0(OpenBeken32s3)",
      },
    }),
    "esp32",
  );
});

test("target resolution selects the matching platform firmware", () => {
  assert.deepEqual(
    resolveUpdateTarget(
      {
        esp8266: {
          otaUrl: "http://example.test/OpenBeken.bin.gz",
          targetVersion: "14.4.1",
        },
        esp32: {
          otaUrl: "http://example.test/OpenBeken32.bin",
          targetVersion: "14.4.2",
        },
      },
      {
        StatusFWR: {
          Hardware: "ESP32-D0WDQ6",
          Version: "14.4.0(OpenBeken32)",
        },
      },
    ),
    {
      otaUrl: "http://example.test/OpenBeken32.bin",
      targetVersion: "14.4.2",
    },
  );
});

test("target resolution falls back to the default target for manual uploads", () => {
  assert.deepEqual(
    resolveUpdateTarget(
      {
        default: {
          otaUrl: "http://example.test/upload.bin",
          targetVersion: "",
        },
      },
      {
        StatusFWR: {
          Hardware: "ESP8285",
          Version: "14.4.0(OpenBeken)",
        },
      },
    ),
    {
      otaUrl: "http://example.test/upload.bin",
      targetVersion: "",
    },
  );
});

test("legacy ESP8266 auto updates are blocked with manual path guidance", () => {
  assert.deepEqual(
    determineUpgradePlan(
      {
        source: "automatic",
        otaUrl: "http://example.test/OpenBeken.bin.gz",
        minimalOtaUrl: "http://example.test/OpenBeken-minimal.bin.gz",
        targetVersion: "14.4.2",
      },
      {
        StatusFWR: {
          Hardware: "ESP8266EX",
          Version: "6.5.0(OpenBeken)",
        },
      },
    ),
    {
      type: "blocked",
      key: "BLOCK_UPDATE_LEGACY_PATH_REQUIRED",
      values: ["6.5.0(OpenBeken)", "14.4.2", "9.1.3"],
    },
  );
});

test("ESP8266 auto updates can stage minimal before final once legacy multi-hop is not required", () => {
  assert.deepEqual(
    determineUpgradePlan(
      {
        source: "automatic",
        otaUrl: "http://example.test/OpenBeken.bin.gz",
        minimalOtaUrl: "http://example.test/OpenBeken-minimal.bin.gz",
        targetVersion: "14.4.2",
      },
      {
        StatusFWR: {
          Hardware: "ESP8285",
          Version: "9.1.3(OpenBeken)",
        },
      },
    ),
    {
      type: "staged",
      steps: [
        {
          kind: "minimal",
          otaUrl: "http://example.test/OpenBeken-minimal.bin.gz",
          targetVersion: "",
        },
        {
          kind: "final",
          otaUrl: "http://example.test/OpenBeken.bin.gz",
          targetVersion: "14.4.2",
        },
      ],
    },
  );
});

test("legacy ESP8266 manual uploads still use the staged minimal path", () => {
  assert.deepEqual(
    determineUpgradePlan(
      {
        source: "manual",
        otaUrl: "http://example.test/OpenBeken.bin.gz",
        minimalOtaUrl: "http://example.test/OpenBeken-minimal.bin.gz",
        targetVersion: "14.4.2",
      },
      {
        StatusFWR: {
          Hardware: "ESP8266EX",
          Version: "6.5.0(OpenBeken)",
        },
      },
    ),
    {
      type: "staged",
      steps: [
        {
          kind: "minimal",
          otaUrl: "http://example.test/OpenBeken-minimal.bin.gz",
          targetVersion: "",
        },
        {
          kind: "final",
          otaUrl: "http://example.test/OpenBeken.bin.gz",
          targetVersion: "14.4.2",
        },
      ],
    },
  );
});

test("direct updates remain unchanged when no minimal stage is required", () => {
  assert.deepEqual(
    determineUpgradePlan(
      {
        otaUrl: "http://example.test/OpenBeken32.bin",
        targetVersion: "14.4.2",
      },
      {
        StatusFWR: {
          Hardware: "ESP32-D0WDQ6",
          Version: "14.4.0(OpenBeken32)",
        },
      },
    ),
    {
      type: "direct",
      steps: [
        {
          kind: "final",
          otaUrl: "http://example.test/OpenBeken32.bin",
          targetVersion: "14.4.2",
        },
      ],
    },
  );
});
