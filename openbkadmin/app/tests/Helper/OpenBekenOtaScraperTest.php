...
class OpenBekenOtaScraperTest extends TestCase
{
    public function testGetStableEsp8266Firmware(): void
    {
        $scraper = new OpenBekenOtaScraper('stable', new HttpBrowser($this->getHttpClient()));
        ...
        self::assertSame('OpenBeken-display.bin.gz', $result->getFirmwares()[0]->getName());
        self::assertStringStartsWith('http', $result->getFirmwares()[0]->getUrl());
    }

    public function testGetDevEsp8266Firmware(): void
    {
        $scraper = new OpenBekenOtaScraper('dev', new HttpBrowser($this->getHttpClient()));
        $result = $scraper->getEsp8266Firmware();
    ...
    public function testGetStableEsp32Firmware(): void
    {
        ...
        self::assertNotEmpty($result->getFirmwares());
        self::assertSame('OpenBeken32-bluetooth.bin', $result->getFirmwares()[0]->getName());
        self::assertStringStartsWith('http', $result->getFirmwares()[0]->getUrl());
    }
...
class MockClient extends MockHttpClient
{
    ...
    private function handleRequests(string $method, string $url): MockResponse
    {
        ...
        if ('GET' === $method && 'https://ota.OpenBeken.com/OpenBeken/' === $url) {
        ...
        if ('GET' === $method && 'https://ota.OpenBeken.com/OpenBeken32/' === $url) {
