...
function initCommandHelper() {
  ...
  $(".applyBatchAction").on("click", function () {
    ...
    $.each(selectedDevices, function (idx, device_id) {
      OpenBeken.generic(device_id, command, undefined, function (result) {
...
function processRow($tr) {
  return new Promise((resolve) => {
    ...
    if (!$tr.hasClass("updating")) {
      ...
      OpenBeken.getStatus(device_ip, device_id, function (data) {
        if (
          data &&
          !data.ERROR &&
          !data.WARNING &&
          ...
          if (device_group === "multi") {
            $(
              ...
              let device_status = OpenBeken.parseDeviceStatus(data, device_relais);
...
function updateAllStatus() {
  ...
  if (!device_holder.hasClass("updating")) {
    ...
    OpenBeken.getAllStatus(timeout, function (result) {
...
function deviceTools() {
  ...
  $("#device-list tbody tr td.status .form-switch").on("click", function (e) {
    ...
    void confirmAction({
      ...
      onConfirm: function () {
        ...
        OpenBeken.toggle(device_ip, device_id, device_relais, function (data) {
  ...
  $("#deleteDeviceModal .btn-ok").on("click", function () {
    ...
    if (action === "restart") {
      ...
      OpenBeken.generic(device_id, "Restart", 1);
      return;
    }
  ...
  $(document).on("focusout keypress", ".dblEdit-Input", function (e) {
    ...
    if (input.val() !== "") {
      ...
      $(this)
        ...
        .removeClass("dont-update");
      if (target == "device") {
        OpenBeken.updateConfig(device_id, cmnd, newvalue, updateStatus);
      } else if (target == "csv") {
        OpenBeken.setDeviceValue(device_id, field, newvalue, td);
      }
    } else {
...
function updateRow(row, data, device_status) {
  ...
  data = OpenBeken.parseStatusData(data);
  if (data.hasWifi) {
  ...
  const rawVersion = data.StatusFWR.Version ?? "?";
  let chipset = "?";
  let version = rawVersion;
  const openBekenVersionMatch = String(rawVersion).match(/^Open([^_]+)_(.+)$/i);
  if (openBekenVersionMatch) {
    chipset = openBekenVersionMatch[1];
    version = openBekenVersionMatch[2];
  }
  $(row).find(".chipset span").html(chipset);
  setSortAttribute(row, ".chipset span", "data-sort-text", chipset);
  ...
  let device_hostname = OpenBeken.parseDeviceHostname(data);
