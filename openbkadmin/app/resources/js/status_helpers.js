...
function normalizeWifiPercent(rssi) {
  if (rssi === undefined || rssi === null || rssi === "") return null;
  const value = Number(rssi);
  if (!Number.isFinite(value)) return null;

  // Some OpenBeken targets expose RSSI as dBm, others already expose a percent.
  // Convert dBm using the common Wi-Fi approximation and always clamp to 0..100.
  const percent = value < 0 ? 2 * (value + 100) : value;
  return Math.max(0, Math.min(100, Math.round(percent)));
}

function getWifiDisplayInfo(data = {}) {
  if (!data.hasWifi || data.wifi === undefined) {
    return { summary: "-", details: "", tooltip: "" };
  }

  const wifiPercent = normalizeWifiPercent(data.wifi.rssi);
  const summary = wifiPercent === null ? "?" : `${wifiPercent}%`;
  const details = [
    data.wifi.ssid,
    data.wifi.channel ? `ch${data.wifi.channel}` : "",
  ].filter(Boolean).join(" / ");

  return { summary, details, tooltip: [details, summary].filter(Boolean).join(" / ") };
}

function parseUptimeToSeconds(uptime) {
  if (typeof uptime !== "string") return null;
  const normalizedUptime = uptime.trim();
  const match = normalizedUptime.match(/^(?:(?<days>\d+)T)?(?<hours>\d{1,2}):(?<minutes>\d{2}):(?<seconds>\d{2})$/);
  ...
  if ([days, hours, minutes, seconds].some(Number.isNaN)) return null;
  return days * 86400 + hours * 3600 + minutes * 60 + seconds;
}

function formatUptimeSeconds(secNum, labels) {
  ...
  return (
    ...
    ((hours || days) ? `${hours}${labels.hour} ` : "") +
    ((minutes || hours || days) ? `${minutes}${labels.minute} ` : "") +
    ((seconds || minutes || hours || days) ? `${seconds}${labels.second}` : "-")
  ).trim();
}
...
function getSortableVersionValue(version) {
  ...
  if (match === null) return null;
  return match[1].split(".").map((part) => part.padStart(6, "0")).join(".");
}

function getRuntimeInfo(data, labels, now = new Date()) {
  const normalized = ensureStatusSections(data);
  const startup = normalized.StatusPRM.StartupDateTimeUtc ?? normalized.StatusPRM.StartupUTC ?? "";
  ...
  if (startup !== "") {
    ...
    if (!Number.isNaN(startupDateTime.getTime())) {
      ...
      return { text: formatUptimeSeconds(secNum, labels), sortValue: secNum, startupDateTime };
    }
  }

  const fallbackText = normalized.StatusSTS.Laufzeit ?? normalized.StatusSTS.Uptime ?? normalized.StatusPRM.Uptime ?? "?";
  const fallbackSeconds = parseUptimeToSeconds(fallbackText);
  return {
    text: fallbackSeconds === null ? fallbackText : formatUptimeSeconds(fallbackSeconds, labels),
    sortValue: fallbackSeconds,
...
function getIlluminance(data, joinString = "<br/>") {
  ...
  Object.values(normalized.StatusSNS).forEach((sensor) => {
    if (sensor !== null && typeof sensor === "object" && sensor.Illuminance !== undefined) {
...
function getEnergyPower(data, joinString = "<br/>") {
  ...
  if (energy.Power !== undefined) energyPower.push(`${energy.Power} W`);
  if (energy.Today !== undefined) {
    const totals = [energy.Today];
    if (energy.Yesterday !== undefined) totals.push(energy.Yesterday);
    if (energy.Total !== undefined) totals.push(energy.Total);
    energyPower.push(`${totals.join(" / ")} kWh`);
  }
  if (energy.Current !== undefined) energyPower.push(`${energy.Current} A`);
