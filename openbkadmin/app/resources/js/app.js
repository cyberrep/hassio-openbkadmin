import { OpenBeken as OpenBekenClient } from "./OpenBeken";
import {
...
const OpenBeken = new OpenBekenClient({
  base_url: config.base_url,
  timeout: 15,
});
window.OpenBeken = OpenBeken;
...
export function getTemp(data, joinString) {
  ...
  if (data.StatusSNS.DHT11 !== undefined) {
    temp.push(data.StatusSNS.DHT11.Temperature + "°" + data.StatusSNS.TempUnit);
  }
  ...
  if (data.StatusSNS.SHT3X !== undefined) {
    temp.push(data.StatusSNS.SHT3X.Temperature + "°" + data.StatusSNS.TempUnit);
  }
  ...
  if (data.StatusSNS["BME280-76"] !== undefined) {
    temp.push(
      data.StatusSNS["BME280-76"].Temperature + "°" + data.StatusSNS.TempUnit,
    );
  }
  if (data.StatusSNS["BME280-77"] !== undefined) {
    temp.push(
      data.StatusSNS["BME280-77"].Temperature + "°" + data.StatusSNS.TempUnit,
    );
  }
  ...
  if (data.StatusSNS.HTU21 !== undefined) {
    temp.push(data.StatusSNS.HTU21.Temperature + "°" + data.StatusSNS.TempUnit);
  }
  ...
  if (data.StatusSNS.AHT1X !== undefined) {
    temp.push(data.StatusSNS.AHT1X.Temperature + "°" + data.StatusSNS.TempUnit);
  }
...
function checkForUpdate(timer) {
  ...
  var githubApiRelease =
    "https://api.github.com/repos/OpenBKAdmin/OpenBKAdmin/" + action;
  ...
  $.get(
        ...
        if (result.tag_name !== undefined) {
          ...
          if (latestTag !== currentGitTag) {
            ...
            if (result.assets.length !== 3) {
              console.log(
                "[APP][checkForUpdate] Seems like Travis is not done yet",
              );
          ...
          if (result.message !== undefined) {
            icon
              ...
              .addClass("fa-times");
            console.log(
              "[APP][checkForUpdate] Github Error => " + result.message,
            );
    ...
    console.log(
      "[APP][checkForUpdate] Github Error => " +
        result.status +
        ": " +
