...
class OpenBekenHelper
{
    private const CHANGELOG_URLS = [
        'dev' => 'https://raw.githubusercontent.com/arendst/OpenBeken/development/CHANGELOG.md',
        'stable' => 'https://raw.githubusercontent.com/arendst/OpenBeken/master/CHANGELOG.md',
    ];
    ...
    {
        $releaseLog = $this->getContents('https://raw.githubusercontent.com/arendst/OpenBeken/development/RELEASENOTES.md');
        $releaseLog = str_replace(['*/', '/*', " *\n"], ['', '', ''], $releaseLog);
        $releaseLog = str_replace(
            'https://github.com/arendst/OpenBeken/blob/master/tools/logo/OpenBeken_FullLogo_Vector.svg',
    ...
    {
        return $this->getReleaseNames($this->OpenBekenOtaScraper->getEsp8266Firmware(), '.bin.gz', '-minimal.bin');
    }

    public function getEsp32Releases(): array
    {
        return $this->getReleaseNames($this->OpenBekenOtaScraper->getEsp32Firmware(), '.bin');
    }

    public function getLatestFirmwares(string $configuredFirmware): AutoFirmwareResult
    {
        $isEsp32 = str_starts_with($configuredFirmware, 'OpenBeken32');
    ...
    {
        $firmwareResult = $this->OpenBekenOtaScraper->getEsp32Firmware();
    ...
    {
        $firmwareResult = $this->OpenBekenOtaScraper->getEsp8266Firmware();
        foreach ($firmwareResult->getFirmwares() as $asset) {
            if ('OpenBeken-minimal.bin.gz' === $asset->getName()) {
    ...
    {
        $OpenBekenIssueUrl = 'https://github.com/arendst/OpenBeken/issues/';

        return preg_replace(
            '/\B#([\d]+)/',
            "<a href='{$OpenBekenIssueUrl}$1' target='_blank'>#$1</a>",
            $content
        );
    }

    private function getReleaseNames(OpenBekenFirmwareResult $firmwareResult, string $suffix, ?string $excludedPart = null): array
    {
        $releases = [];
        foreach ($firmwareResult->getFirmwares() as $asset) {
